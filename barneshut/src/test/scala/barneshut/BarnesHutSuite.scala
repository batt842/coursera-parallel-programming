...
class BarnesHutSuite extends FunSuite {
  ...
  test("Leaf.insert(b) should return a new Fork if size > minimumSize") {
    val b1 = new Body(1f, 1f, 1f, 0f, 0f)
    val b2 = new Body(1f, 1f + minimumSize, 1f, 0f, 0f)
    val b3 = new Body(1f, 1f, 1f + minimumSize, 0f, 0f)

    val quad = Leaf(1f, 1f, minimumSize * 2, Seq(b1, b2))
    val newQuad = quad.insert(b3)
