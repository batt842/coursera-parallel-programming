...
class KMeans {
  ...
  def classify(points: GenSeq[Point], means: GenSeq[Point]): GenMap[Point, GenSeq[Point]] = {
    val naiveResult = points.groupBy(findClosest(_, means))
    val emptyMeans = means.filterNot(naiveResult.contains(_)).map(_ -> GenSeq()).toMap
    naiveResult ++ emptyMeans
  }
  ...
  def converged(eta: Double)(oldMeans: GenSeq[Point], newMeans: GenSeq[Point]): Boolean = {
    oldMeans.zip(newMeans).forall(pp => pp._1.squareDistance(pp._2) <= eta)
  }
